...
void Cookieboy::GPU::RenderScanline()
{
	...
	{
		if (WX <= 166 && WY <= 143 && WndPos <= 143 && LY >= WY)//Checking window visibility on the whole screen and in the scanline
		{
			WORD tileMap = (LCDC & 0x40) ? 0x1C00 : 0x1800;
			WORD tileData = (LCDC & 0x10) ? 0x0 : 0x800;

			int windowX = (signed)WX - 7;//This can be negative. Window is offset 7 pixels
			WORD tileMapX = 0;
			WORD tileMapY = ((WndPos) >> 3) & 0x1F;
			BYTE tileMapTileX = 0;
			BYTE tileMapTileY = (WndPos) & 0x7;
